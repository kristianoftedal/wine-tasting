...
import { serverSideSimilarity } from "./similarity"
...
export async function findSimilarWinesSQL(
  ...
  for (const candidate of candidateWines.slice(0, limit * 2)) {
    ...
    if (combinedSmell && wine.smell) {
      smellSimilarity = await serverSideSimilarity(combinedSmell, wine.smell)
    }

    if (combinedTaste && wine.taste) {
      tasteSimilarity = await serverSideSimilarity(combinedTaste, wine.taste)
    }
