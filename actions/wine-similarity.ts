...
export type { RecommendationWeights, RecommendationThresholds, WineSimilarityScore }
...
export async function findSimilarWines(
  ...
  const highRatedWineIds = highRatedTastings.map((t) => t.wine_id)
  ...
  const tastedWineIds = new Set(allUserTastings?.map((t) => t.wine_id) || [])
