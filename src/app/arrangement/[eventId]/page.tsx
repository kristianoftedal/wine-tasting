...
export default async function Arrangement({ params }: { params: Promise<{ eventId: string }> }) {
  ...
  const tastedWineIds = new Set(userTastings?.map((t) => t.product_id) || [])

  const sortedWines = wines?.sort((a, b) => event.wines.indexOf(a.product_id) - event.wines.indexOf(b.product_id)) || []

  const nextSuggestedWine = sortedWines.find((wine) => !tastedWineIds.has(wine.product_id))
  ...
  return (
    <div className={styles.container}>
      ...
      <Link href={`/arrangement/${eventId}/resultater`} className={styles.resultsLink}>
      ...
      <section className={styles.winesSection}>
        ...
        <div className={styles.winesList}>
          {sortedWines.map((wine: Wine, index: number) => {
            const isTasted = tastedWineIds.has(wine.product_id)
            const isNextSuggested = nextSuggestedWine?.product_id === wine.product_id
            ...
            return (
              <NavButton
                ...
                eventId={eventId}
                year={wine.year}
                key={`${wine.product_id}-${wine.year || index}`}
              >
                <article
