...
export type Wine = {
  ...
  fylde: number | null // Body/fullness (integer 1-12)
  friskhet: number | null // Freshness/acidity (integer 1-12)
  garvestoff: number | null // Tannin (integer 1-12, red wines only)
  sodme: number | null // Sweetness (integer 1-12, white/sparkling wines only)
  sugar: string
  acidity: string
  grapes: string[]
  is_good_for: string
...
export type Tasting = {
  id: string
  user_id: string
  wine_id: string // Changed from product_id to wine_id (UUID)
  ...
  // Numeric characteristics (integers)
  friskhet: number | null // Freshness (integer 1-12)
  fylde: number | null // Body (integer 1-12)
  sodme: number | null // Sweetness (integer 1-12)
  snaerp: number | null // Tannin (integer 1-12)
  karakter: number | null // Overall character (integer 1-10)
...
export type TastingFormData = {
  ...
  userId: string
  wineId: string // Changed from productId to wineId (UUID)
  tastedAt: Date
...
export function tastingFormToDb(form: TastingFormData): Omit<Tasting, "id" | "created_at" | "updated_at"> {
  ...
  return {
    user_id: form.userId,
    wine_id: form.wineId, // Changed from product_id to wine_id
...
export function tastingDbToForm(db: Tasting): TastingFormData {
  ...
  return {
    ...
    selectedFlavorsSmak,
    userId: db.user_id,
    wineId: db.wine_id, // Changed from productId to wineId
...
export const initialTastingForm: TastingFormData = {
  ...
  smaksIntensitet: "",
  userId: "",
  wineId: "", // Changed from productId to wineId
  alkohol: "",
  pris: 0,
  tastedAt: new Date(),
  colorScore: 0,
